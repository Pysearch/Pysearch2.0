...
def test_loading_results_view_redirects(dummy_request):
    """Test that loading results redirects and has an url attached."""
    from pysearch.views.default import loading_view
    from pyramid.httpexceptions import HTTPFound
    dummy_request.params['url'] = 'https://www.sample.com'
    result = loading_view(dummy_request)
    assert isinstance(result, HTTPFound)


def test_about_view_is_returns_empty_dict(dummy_request):
    """Test the about view returns an empty dict."""
    from pysearch.views.default import about_view
